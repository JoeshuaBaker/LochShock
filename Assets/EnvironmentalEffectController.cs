...
public class EnvironmentalEffectController : MonoBehaviour
{
    ...
    private void UpdateLightningLight()
    {
        ...
        isBaseLightningChanceScaling = baseLightningChance * Mathf.Min((gameTime * (baseLightningMax / 2f) / maxTimeScaling) + (timeSinceOrbUsed * (baseLightningMax / 2f) / orbResetTime), baseLightningMax);
        ...
        if (lightningChance < baseLightningChance *  Mathf.Min((gameTime * (baseLightningMax /2f) / maxTimeScaling) + (timeSinceOrbUsed * (baseLightningMax / 2f) / orbResetTime) , baseLightningMax))
        {
            ...
            isLightningStrikeScaling = lightningStrikeChance * Mathf.Min((gameTime * ((lightningStrikeMax / 2f) / maxTimeScaling)) + (timeSinceOrbUsed * ((lightningStrikeMax / 2f) / orbResetTime)), lightningStrikeMax);
            isLightningLargeScaling = lightningStrikeChance * lightningLargeChance * Mathf.Min((gameTime * ((lightningLargeMax / 2f) / maxTimeScaling)) + (timeSinceOrbUsed * ((lightningLargeMax / 2f) / orbResetTime)), lightningLargeMax);
            isLightningMediumScaling = lightningStrikeChance * lightningMediumChance * Mathf.Min((gameTime * ((lightningMediumMax / 2f) / maxTimeScaling)) + (timeSinceOrbUsed * ((lightningMediumMax / 2f) / orbResetTime)), lightningMediumMax);
            ...
            else if (lightningPick < lightningStrikeChance * lightningLargeChance * Mathf.Min((gameTime * ((lightningLargeMax / 2f) / maxTimeScaling)) + (timeSinceOrbUsed * ((lightningLargeMax / 2f) / orbResetTime)), lightningLargeMax))
            {
               // Debug.Log("lightningLarge" + lightningPick);
                lightningAnimator.SetBool("LightningLarge", true);
            }
            else if (lightningPick < lightningStrikeChance * lightningMediumChance * Mathf.Min((gameTime * ((lightningMediumMax / 2f) / maxTimeScaling)) + (timeSinceOrbUsed * ((lightningMediumMax / 2f) / orbResetTime)), lightningMediumMax))
            {
    ...
    private void UpdateRain()
    {
        ...
        emission.rateOverTime = Mathf.Min((gameTime * ( 750f / maxTimeScaling )) + (timeSinceOrbUsed * (250f / orbResetTime)) , 1000f );
    }
}
